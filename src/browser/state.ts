...
export async function ensurePage(): Promise<Page> {
  if (page) {
    ...
    try {
      ...
      const { checkDomain } = await import('../util/domain-filter.js');
      ...
      const { logCritical } = await import('../log.js');
  ...
  try {
    ...
    throw new Error(
      `No active page and reconnection failed: ${(err as Error).message}. Use browser_navigate first.`
    );
  }
...
export function addConsoleLog(entry: ConsoleEntry): void {
  ...
  if (consoleLogs.length > logBufferMax) {
    // Ring buffer: slice creates new array but keeps memory bounded
    consoleLogs = consoleLogs.slice(-logBufferMax);
  }
}
...
export function addErrorLog(entry: ErrorEntry): void {
  ...
  if (errorLogs.length > logBufferMax) {
    errorLogs = errorLogs.slice(-logBufferMax);
  }
}
