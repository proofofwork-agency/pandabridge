...
export async function reconnectFresh(config: Config): Promise<void> {
  ...
  if (currentBrowser) {
    try {
      ...
      logCritical(`Browser close error during reconnect: ${(err as Error).message}`);
    }
  }
...
const listenedBrowsers = new WeakSet<import('playwright-core').Browser>();
const expectedDisconnects = new WeakSet<import('playwright-core').Browser>();
...
const requestIdMap = new WeakMap<import('playwright-core').Request, string>();
