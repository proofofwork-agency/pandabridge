    ...
    async ({ url, symptom, actions }) => {
      try {
        ...
        try {
          ...
          if (!(err instanceof Error) || !isRecoverableError(err)) throw err;
          await debugReportDeps.reconnectFresh(config);
          page = await debugReportDeps.ensurePage();
          const response = await page.goto(url, { waitUntil: 'domcontentloaded', timeout: config.defaultTimeout });
          setNavigated(true);
        ...
        const baseline = {
          ...
          consoleCount: getConsoleLogs().length,
          networkCount: getNetworkLogs().length,
        };
        ...
        if (errors.length > 0) {
          ...
          for (const e of capped) {
            const typeLabel = e.type === 'framework'
              ? `${e.framework ?? 'framework'}-error`
              : e.type === 'rejection' ? 'rejection' : 'error';
            let location = '';
            if (e.source) {
              location = ` (${e.source}`;
              if (e.line !== undefined) location += `:${e.line}`;
              if (e.col !== undefined) location += `:${e.col}`;
              location += ')';
            }
            lines.push(`[${typeLabel}] ${e.message}${location}`);
          }
          if (errors.length > 10) lines.push(`... [${errors.length - 10} more errors omitted]`);
          lines.push('');
        }

        if (failedRequests.length > 0) {
          lines.push(`## Failed/Error Requests (${failedRequests.length})`);
          const capped = failedRequests.slice(0, 10);
          ...
          if (failedRequests.length > 10) lines.push(`... [${failedRequests.length - 10} more entries omitted]`);
          lines.push('');
        }

        if (consoleIssues.length > 0) {
          lines.push(`## Console Issues (${consoleIssues.length})`);
          const capped = consoleIssues.slice(0, 10);
          for (const c of capped) {
            lines.push(`[${c.level}] ${c.text}`);
          }
          if (consoleIssues.length > 10) lines.push(`... [${consoleIssues.length - 10} more entries omitted]`);
          lines.push('');
        }
        ...
        return {
          content: [{ type: 'text' as const, text: `Error generating debug report for ${url}: ${(err as Error).message}` }],
          isError: true,
        };
      }
    }
