...
import { matchesDomain } from '../util/domain-filter.js';
      ...
      try {
        ...
        const filtered = allLinks
          ...
          .filter((l) => {
            ...
            try {
              return matchesDomain(new URL(l.href).hostname, domain);
            } catch {
              return false;
            }
          });
        ...
        return {
          content: [{ type: 'text', text: `Error extracting links: ${toErrorMessage(err)}` }],
          isError: true,
        };
      }
    }
