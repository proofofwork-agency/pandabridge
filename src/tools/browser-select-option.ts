  ...
  server.tool(
    ...
    async ({ selector, elementId, value }) => {
      try {
        ...
        await waitForSettle(page, Math.min(config.defaultTimeout, 1500));
        ...
        return {
          content: [{ type: 'text', text: `Error selecting option on ${elementId ?? selector ?? '<missing target>'}: ${(err as Error).message}` }],
          isError: true,
        };
      }
    }
